...
namespace TurrantKar.EH
{
    ...
    /// <summary>
    /// This class globaly handle all types of exception and retun TK error object in response.
    ...
    const string _messageStringTemplate =
      "HTTP {RequestMethod} {RequestPath} responded {StatusCode} {TKError} from Machine: {machine} UserId: {UserId}, UserName: {UserName}, TenantId: {TenantId} and TenantName: {TenantName} {NewLine}";
    ...
    /// [a] Parse exception to TK error object
    /// [b] Log exception 
    /// [c] Serialize TK object to json
    /// [d] Write TK json to http response 
    /// </summary>
    ...
    private Task HandleExceptionAsync(HttpContext context, Exception ex) {
      ...
      IList<TKErrorData> errorDataList = new List<TKErrorData>();
      ...
      // Invalid version          
      if(originalEx is TKInvalidVersionException) {
        messages.Add(originalEx.Message);
        errorType = ErrorType.InvalidVersion;
        TKErrorData data = new TKErrorData((int)InvalidVersionErrorSubType.ApplicationVersion, "", "Invalid application version");
        errorDataList.Add(data);
      }

      // System Exception          
      else if(originalEx is TKSystemException) {
        messages.Add(originalEx.Message);
        errorType = ErrorType.System;
        errorDataList.Add(new TKErrorData() {
      ...
      else if(originalEx is TKInvalidDBVersionException) {
        TKInvalidDBVersionException dbVerEx = (TKInvalidDBVersionException)originalEx;
        messages.Add(originalEx.Message);
        errorType = ErrorType.InvalidVersion;
        TKErrorData data = new TKErrorData((int)InvalidVersionErrorSubType.DatabaseVersion, dbVerEx.LatestDBVersion.ToString(), "Invlid DB version");
        errorDataList.Add(data);
      }
      // Invalid application version
      else if(originalEx is TKInvalidAppVersionException) {
        TKInvalidAppVersionException invalidAppVersionEx = (TKInvalidAppVersionException)originalEx;
        messages.Add(originalEx.Message);
        errorType = ErrorType.InvalidVersion;
      ...
      // Database Exception   
      else if(originalEx is TKDatabaseException) {
        TKDatabaseException databaseEx = (TKDatabaseException)originalEx;
        messages.Add(originalEx.Message);
      ...
      // Invalid token
      else if(originalEx is TKInvalidLoginTokenException) {
        messages.Add(originalEx.Message);
      ...
      // Invalid Login Email
      else if(originalEx is TKInvalidLoginEmailIdException) {
        messages.Add(originalEx.Message);
        errorType = ErrorType.Authentication;
        errorDataList.Add(new TKErrorData() {
          Data = "Email",
      ...
      // Invalid Password
      else if(originalEx is TKInvalidPasswordException) {
        messages.Add(originalEx.Message);
        errorType = ErrorType.Authentication;
        errorDataList.Add(new TKErrorData() {
          Data = "Password",
      ...
      else if(originalEx is TKUserLockedException) {
        messages.Add(originalEx.Message);
        errorType = ErrorType.Authentication;
        errorDataList.Add(new TKErrorData() {
          Data = "UserLocked",
      ...
      // Security Exception. 
      else if(originalEx is TKSecurityException) {
        TKSecurityException securityEx = (TKSecurityException)originalEx;
        messages.Add(originalEx.Message);
        errorType = ErrorType.Security;
        errorDataList = securityEx.ErrorDataList;
      }
      // Validation Exception like required fields.
      else if(originalEx is TKValidationException) {
        TKValidationException validationEx = (TKValidationException)originalEx;
        messages.Add(originalEx.Message);
        errorType = ErrorType.Validation;
        errorDataList = validationEx.ErrorDataList;
      }
      // Duplicate Name Exception.
      else if(originalEx is TKDuplicateNameException) {
        TKDuplicateNameException duplciateNameEx = (TKDuplicateNameException)originalEx;
        messages.Add(originalEx.Message);
        errorType = ErrorType.Duplicate;
        errorDataList = duplciateNameEx.ErrorDataList;
      }
      // DataConcurrency Excetpion.
      else if(originalEx is TKDataConcurrencyException) {
        TKDataConcurrencyException dataConcurrancyEx = (TKDataConcurrencyException)originalEx;
        messages.Add(originalEx.Message);
        errorType = ErrorType.Concurrency;
        errorDataList = dataConcurrancyEx.ErrorDataList;
      }
      // Import Data Exception.
      else if(originalEx is TKImportDataException) {
        TKImportDataException importDataEx = (TKImportDataException)originalEx;
        messages.Add(originalEx.Message);
      ...
      else if(originalEx is TKInvalidDeviceIdException) {
        TKInvalidDeviceIdException invalidDeviceIdEx = (TKInvalidDeviceIdException)originalEx;
        messages.Add(originalEx.Message);
        errorType = ErrorType.InvalidDeviceId;
        errorDataList = invalidDeviceIdEx.ErrorDataList;
      }
      // Null Request Argument Exception.
      else if(originalEx is TKNullRequestArgumentException) {
        messages.Add(originalEx.Message);
        errorType = ErrorType.NullRequestArgument;
        errorDataList = (originalEx as TKNullRequestArgumentException).ErrorDataList;
      }
      // Invalid remote device or device not register on local.
      else if(originalEx is TKInvalidTimeZoneException) {
        TKInvalidTimeZoneException invalidTimeZoneEx = (TKInvalidTimeZoneException)originalEx;
        messages.Add(originalEx.Message);
        errorType = ErrorType.InvalidTimeZone;
        errorDataList = invalidTimeZoneEx.ErrorDataList;
      }
      // Invalid field reference exception.
      else if(originalEx is TKInvalidFieldReferenceException) {
        TKInvalidFieldReferenceException invalidFieldRefEx = (TKInvalidFieldReferenceException)originalEx;
        messages.Add(originalEx.Message);
        errorType = ErrorType.InvalidFieldReference;
        errorDataList = invalidFieldRefEx.ErrorDataList;
      }
      // Invalid operation exception.
      else if(originalEx is TKInvalidOperationException) {
        TKInvalidOperationException invalidOperationEx = (TKInvalidOperationException)originalEx;
        messages.Add(originalEx.Message);
        errorType = ErrorType.InvalidOperation;
        errorDataList = invalidOperationEx.ErrorDataList;
      }
      // Invalid operation exception.
      else if(originalEx is TKDowntimeException) {
        TKDowntimeException downtimeException = (TKDowntimeException)originalEx;
        messages.Add(originalEx.Message);
        errorType = ErrorType.DowntimeException;
        errorDataList = downtimeException.ErrorDataList;
      }
      // Recoverable Exception.
      else if(originalEx.GetType().BaseType == typeof(TKRecoverableException)) {
        messages.Add(originalEx.Message);
        errorType = ErrorType.Validation;
      }
      // For email server is down
        ...
        foreach(var entry in concurrencyEx.Entries) {
          ...
          foreach(var property in entry.CurrentValues.Properties) {
            errorDataList.Add(new TKErrorData() { Message = string.Format(ErrorMessages.FieldConcurrentMessage, property), ErrorSubType = (int)ConcurrencyErrorSubType.None, Data = property });
          }
        }
      }
        ...
        foreach(var entry in dbUpdateException.Entries) {
          errorDataList.Add(new TKErrorData() { Message = string.Format(ErrorMessages.DatabaseOperationErrorMessage, entry.Entity.GetType().Name), ErrorSubType = (int)ConcurrencyErrorSubType.None, Data = entry.Entity.GetType().Name });
        }
      }
        ...
        if(originalEx.GetBaseException().Message.Contains("Violation of UNIQUE KEY constraint 'UQ_Adderss_ParentEntityId_Label_AddressType'")) {
          ...
          for(int i = 0; i < sqlEx.Errors.Count; i++) {
            ...
            errorDataList.Add(new TKErrorData() { Message = errorMessages.ToString(), ErrorSubType = (int)ConcurrencyErrorSubType.None, Data = sqlEx.Errors[i].Procedure });
          }
        }
      }
      ...
      else if(originalEx is TKServiceException) {
        TKServiceException serviceException = (TKServiceException)originalEx;
        messages.Add(originalEx.Message);
        errorType = serviceException.GetTKErrorType();
      ...
      if(errorDataList == null || errorDataList.Count == 0) {
        errorDataList = new List<TKErrorData>();
        TKErrorData TKErrorData = new TKErrorData();
        TKErrorData.Message = originalEx.Message;
        ...
        TKErrorData.Data = errorData;
        errorDataList.Add(TKErrorData);
      }

      TKError error = new TKError(errorType, messages, errorDataList);

      // Parse object in json
      string jsonError = TKError.ToJSON(error);
    ...
    /// <returns></returns>
    private bool LogException(HttpContext httpContext, Exception ex, string TKError) {
      ...
      Log.Error(ex, _messageStringTemplate, httpContext.Request.Method, httpContext.Request.Path, 500, TKError, httpContext.Connection.RemoteIpAddress, userId, userName, tenantId, tenantName);
  ...
  //  /// <param name="httpContext">The HTTP context.</param>
  //  /// <param name="TKError">The TK error.</param>
  //  private bool SendLogEmail(Exception exception, HttpContext httpContext, string TKError) {
  //    try {
  ...
  //    }
  //    catch(Exception ex) {
  //      LogException(httpContext, ex, TKError);
  //      return false;
  //    }
  //  }
  }
